...
namespace SoundRender
{
    ...
    void ModalSound::init(const std::string &filename, int materialID)
    {
        ...
        auto modelMtlName = modelName + std::string("_") + MaterialConst::names[materialID];
        auto eigenPath = std::string(ASSET_DIR) + std::string("/eigen/") + modelMtlName + std::string(".npz");
        auto ffatPath = std::string(ASSET_DIR) + std::string("/acousticMap/") + modelMtlName + std::string(".npz");
        ...
        Correction::soundScale = Correction::allSoundScales[modelMtlName];
        return;
    }
    ...
    void ModalSound::AdjustSoundScale()
    {
        ...
        float tanHalfFov = std::tan(glm::radians(45.0f) / 2);
        ...
        for (int i = 0; i < mesh_render->triangles.size(); i++)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    auto mode_f = modalInfo.eigenVec[id] * norm.x + modalInfo.eigenVec[id + 1] * norm.y + modalInfo.eigenVec[id + 2] * norm.z;
                    modalInfo.f += mode_f * 0.5f / 8; // force_max = 1.0f;
                }
            }
        ...
        Correction::soundScale = scale_factor / (currMax*0.75 + 0.1f);
    }
    ...
    void ModalSound::PreprocessAllModals(const std::filesystem::path &meshRootPath, const std::filesystem::path &scaleFilePath)
    {
        ...
        for (const auto &entry : std::filesystem::directory_iterator(meshRootPath))
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                std::string modelMtlName = modelName + "_" + MaterialConst::names[i];
                if (Correction::allSoundScales.find(modelMtlName) == Correction::allSoundScales.end())
                {
                    if (needLoad)
                    {
#ifdef _WIN32
                        meshPath = AsciiWStrToStr(entry.path().c_str());
#else
                        meshPath = entry.path().c_str();
#endif
                        mesh = loadOBJ(meshPath);
                    ...
                    Correction::allSoundScales.emplace(modelMtlName, Correction::soundScale);
                    correctionFile << modelMtlName << " " << Correction::soundScale << "\n";
                }
            }
            ++preprocessedCnt;
